fastglmpca_10_factors <- readr::read_rds(
  "~/Documents/data/fastglmpca/experiment_results/pbmc_fastglmpca_fit_10_factors_450_iter_28_cores_dec_23.rds"
)
...
g1 <- ggplot(fastglmpca_df,aes(x = PC1,y = PC2,color = celltype)) +
  ...
  ggtitle("pbmc fastglmpca PCs 1 and 2") +
  cowplot::theme_cowplot(font_size = 10)
g2 <- ggplot(glmpca_df,aes(x = PC1,y = PC2,color = celltype)) +
  geom_point(size = 1) +
  scale_color_manual(values = pbmc_colors) +
  ggtitle("pbmc glmpca PCs 1 and 2") +
  cowplot::theme_cowplot(font_size = 10)

g3 <- ggplot(fastglmpca_df,aes(x = PC3,y = PC4,color = celltype)) +
  geom_point(size = 1) +
  scale_color_manual(values = pbmc_colors) +
  ggtitle("pbmc fastglmpca PCs 3 and 4") +
  cowplot::theme_cowplot(font_size = 10)
g4 <- ggplot(glmpca_df,aes(x = PC3,y = PC4,color = celltype)) +
  geom_point(size = 1) +
  scale_color_manual(values = pbmc_colors) +
  ggtitle("pbmc glmpca PCs 3 and 4") +
  cowplot::theme_cowplot(font_size = 10)

g5 <- ggplot(fastglmpca_df,aes(x = PC5,y = PC6,color = celltype)) +
  geom_point(size = 1) +
  scale_color_manual(values = pbmc_colors) +
  ggtitle("pbmc fastglmpca PCs 5 and 6") +
  cowplot::theme_cowplot(font_size = 10)
g6 <- ggplot(glmpca_df,aes(x = PC5,y = PC6,color = celltype)) +
  geom_point(size = 1) +
  scale_color_manual(values = pbmc_colors) +
  ggtitle("pbmc glmpca PCs 5 and 6") +
  cowplot::theme_cowplot(font_size = 10)

g7 <- ggplot(fastglmpca_df,aes(x = PC7,y = PC8,color = celltype)) +
  geom_point(size = 1) +
  scale_color_manual(values = pbmc_colors) +
  ggtitle("pbmc fastglmpca PCs 7 and 8") +
  cowplot::theme_cowplot(font_size = 10)
g8 <- ggplot(glmpca_df,aes(x = PC7,y = PC8,color = celltype)) +
  geom_point(size = 1) +
  scale_color_manual(values = pbmc_colors) +
  ggtitle("pbmc glmpca PCs 7 and 8") +
  cowplot::theme_cowplot(font_size = 10)

g9 <- ggplot(fastglmpca_df,aes(x = PC9,y = PC10,color = celltype)) +
  geom_point(size = 1) +
  scale_color_manual(values = pbmc_colors) +
  ggtitle("pbmc fastglmpca PCs 9 and 10") +
  cowplot::theme_cowplot(font_size = 10)
g10 <- ggplot(glmpca_df,aes(x = PC9,y = PC10,color = celltype)) +
  geom_point(size = 1) +
  scale_color_manual(values = pbmc_colors) +
  ggtitle("pbmc glmpca PCs 9 and 10") +
  cowplot::theme_cowplot(font_size = 10)
