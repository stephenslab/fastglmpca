...
set.seed(1)
data <- plash::generate_glmpca_data(n = 1500, p = 750, K = 1, link = "log1p")
...
set.seed(1)
data <- fastTopics::simulate_poisson_gene_data(n = 2000, m = 1000, k = 2, s = rep(35, 2000))
data$Y <- data$X
...
fit_2 <- plash::fit_glmpca(
  Y = data$Y, fit0 = fit0_2, tol = 1e-4, algorithm = "ccd", link = "logsp", s = 1.5e3,
...
expected_H <- tcrossprod(nmf_fit$L, nmf_fit$F) / 1.5e3
...
logsp_fit <- plash::fit_glmpca(
  Y = data2$Y, K = 1, tol = 1e-4, algorithm = "ccd", link = "logsp", s =  1,
  control = list(line_search = TRUE, num_iter = 2)
)

expected_H <- crossprod(glmpca_fit$LL, glmpca_fit$FF) - log(1e-10)
actual_H <- crossprod(logsp_fit$LL, logsp_fit$FF)
