...
inline arma::vec solve_pois_reg_log1p_cpp (
    const arma::mat X, 
    const arma::vec y, 
    arma::vec b, 
  ...
  for (int update_num = 1; update_num <= num_iter; update_num++) {
    ...
    for (int idx = 0; idx < num_indices; idx++) {
      ...
      eta = X * b;
      exp_eta = exp(eta);
      current_lik = sum(exp_eta - (y % log(exp_eta - 1)));
...
// [[Rcpp::export]]
arma::mat update_loadings_log1p (
    const arma::mat& F_T,
    arma::mat& L,
    const arma::mat& Y_T,
    const std::vector<int> update_indices,
  ...
  for (int i = 0; i < Y_T.n_cols; i++) {
    ...
    L.col(i) = solve_pois_reg_log1p_cpp (
      F_T, 
      Y_T.col(i),
      L.col(i), 
      update_indices,
...
arma::mat update_factors_log1p (
  ...
  for (int j = 0; j < Y.n_cols; j++) {
    ...
    FF.col(j) = solve_pois_reg_log1p_cpp (
      L_T, 
      Y.col(j),
      FF.col(j), 
      update_indices,
