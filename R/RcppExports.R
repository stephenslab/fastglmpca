# Generated by using Rcpp::compileAttributes() -> do not edit by hand
# Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

update_loadings <- function(F_T, L, Y_T, update_indices, num_iter, line_search, alpha, beta) {
    .Call(`_plash_update_loadings`, F_T, L, Y_T, update_indices, num_iter, line_search, alpha, beta)
}

update_factors <- function(L_T, FF, Y, update_indices, num_iter, line_search, alpha, beta) {
    .Call(`_plash_update_factors`, L_T, FF, Y, update_indices, num_iter, line_search, alpha, beta)
}

update_loadings_irls <- function(F_T, L, Y_T, update_vec, constant_vec, num_iter) {
    .Call(`_plash_update_loadings_irls`, F_T, L, Y_T, update_vec, constant_vec, num_iter)
}

update_factors_irls <- function(L_T, FF, Y, update_vec, constant_vec, num_iter) {
    .Call(`_plash_update_factors_irls`, L_T, FF, Y, update_vec, constant_vec, num_iter)
}

update_loadings_log1p <- function(F_T, L, Y_T, update_indices, num_iter, line_search, alpha, beta) {
    .Call(`_plash_update_loadings_log1p`, F_T, L, Y_T, update_indices, num_iter, line_search, alpha, beta)
}

update_factors_log1p <- function(L_T, FF, Y, update_indices, num_iter, line_search, alpha, beta) {
    .Call(`_plash_update_factors_log1p`, L_T, FF, Y, update_indices, num_iter, line_search, alpha, beta)
}

update_loadings_log1p_sp <- function(F_T, L, Y_T, update_indices, num_iter, line_search, alpha, beta) {
    .Call(`_plash_update_loadings_log1p_sp`, F_T, L, Y_T, update_indices, num_iter, line_search, alpha, beta)
}

update_factors_log1p_sp <- function(L_T, FF, Y, update_indices, num_iter, line_search, alpha, beta) {
    .Call(`_plash_update_factors_log1p_sp`, L_T, FF, Y, update_indices, num_iter, line_search, alpha, beta)
}

update_loadings_logsp <- function(F_T, L, Y_T, s, update_indices, num_iter, line_search, alpha, beta) {
    .Call(`_plash_update_loadings_logsp`, F_T, L, Y_T, s, update_indices, num_iter, line_search, alpha, beta)
}

update_factors_logsp <- function(L_T, FF, Y, s, update_indices, num_iter, line_search, alpha, beta) {
    .Call(`_plash_update_factors_logsp`, L_T, FF, Y, s, update_indices, num_iter, line_search, alpha, beta)
}

update_loadings_sp <- function(F_T, L, Y_T, update_indices, num_iter, line_search, alpha, beta) {
    .Call(`_plash_update_loadings_sp`, F_T, L, Y_T, update_indices, num_iter, line_search, alpha, beta)
}

update_factors_sp <- function(L_T, FF, Y, update_indices, num_iter, line_search, alpha, beta) {
    .Call(`_plash_update_factors_sp`, L_T, FF, Y, update_indices, num_iter, line_search, alpha, beta)
}

