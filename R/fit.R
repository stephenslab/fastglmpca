...
#'   analogous to \code{\link{svd}}); the other parameters (\eqn{X},
#'   \eqn{B}, \eqn{Z}, \eqn{W}); the log-likelihood achieved
#'   (\code{loglik}); information about which columns of \eqn{B} and
#'   \eqn{W} are fixed (\code{fixed_b_cols}, \code{fixed_w_cols}); and a
#'   data frame \code{progress} storing information about the
#'   algorithm's progress after each update.
#'
