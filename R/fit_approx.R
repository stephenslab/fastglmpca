lik_approx <- function(Y, LL, FF, a1, a2) {

  H <- crossprod(LL, FF)
  Y_nz_idx <- which(Y != 0)
  Y_z_idx <- which(Y == 0)
  nz_loglik <- sum(Y[Y_nz_idx] * H[Y_nz_idx] - exp(H[Y_nz_idx]))
  z_loglik <- sum(-a1 * H[Y_z_idx] - a2 * (H[Y_z_idx] ^ 2))
  loglik <- nz_loglik + z_loglik
  return(loglik)

}


#' @title Fit Poisson GLM-PCA Model to Count Data w/ approx
#'
#' @description Fit a Poisson GLM-PCA model to input matrix \code{Y}
#'   by maximum likelihood with an approximation
#'
...
#'
#'
fit_glmpca_pois_approx <- function(
    Y,
    K,
    fit0,
    a1,
  ...
  current_lik <- lik_approx(Y, fit$LL, fit$FF, a1, a2)
  ...
  fit$progress[["approx_loglik"]] <- numeric(max_iter)
  ...
  fit$progress$approx_loglik[1] <- current_lik
  fit$progress$loglik[1] <- lik_glmpca_pois_log(Y, fit$LL, fit$FF, loglik_const)
  ...
  while (!converged && t <= max_iter) {
    ...
    if (length(LL_update_indices) > 0) {
      ...
      #   )
      #
      #   new_lik <- lik_approx(Y, fit$LL, fit$FF, a1, a2)
      #
      # }

      LL <- foreach(i = 1:n, .combine = cbind) %dopar% {
    ...
    if (length(FF_update_indices) > 0) {
      ...
      FF <- foreach(j = 1:p, .combine = cbind) %dopar% {
    ...
    fit$progress$approx_loglik[t + 1] <- current_lik
    fit$progress$loglik[t + 1] <- lik_glmpca_pois_log(
      Y, fit$LL, fit$FF, loglik_const
    )
  ...
  fit$progress$approx_loglik <- fit$progress$approx_loglik[1:t]
